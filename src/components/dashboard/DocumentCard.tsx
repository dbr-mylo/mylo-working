...
interface DocumentCardProps {
  ...
  onClick?: (documentId: string) => void; // Added onClick handler for WriterDashboard
  isDesigner?: boolean;
  showStatus?: boolean;
  status?: string;
...
export const DocumentCard = ({ 
  ...
  const documentStatus = document.status || status;
  const isPublished = documentStatus === 'published';
  
  const handleStatusToggle = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (onToggleStatus) {
      onToggleStatus(document.id, documentStatus || 'draft');
    }
  };
